...
transType tcEnv ty' = transType' ty' []
  where
    ...
    transType' ty@(TypeConstrained [] _) =
      internalError $ "CurryToIL.transType: empty list of constraints in type: " ++ show ty
...
transTVars tcEnv ty' =
  ...
  where
    ...
    build (TypeConstrained (ty:_) _) k =
      build ty k
    build (TypeConstrained [] _) _ =
      internalError $ "CurryToIL.transTVars: empty list of constraints in type: " ++ show ty'
...
trFunction _ _ = internalError "CurryToIL.trFunction: empty equation list"
...
rigidMatch vs alts@(alt:_) = rigidOptMatch (snd alt) id vs (fmap prepare (alt :| alts))
...
rigidOptMatch :: IL.Expression            -- default expression
              ...
              -> NonEmpty Match'          -- translated equations
              -> IL.Expression
-- if there are no variables left: return the default expression
rigidOptMatch def _      []       _    = def
rigidOptMatch def prefix (v : vs) alts
  | isDemanded = rigidMatchDemanded prefix v vs (alt' : alts')
  | otherwise  = rigidOptMatch def (prefix . (v:)) vs (fmap skipPat' alts)
  where
  isDemanded    = not $ isVarMatch alt'
  alt' :| alts' = fmap tagAlt' alts
