...
data OptimizationOpts = OptimizationOpts
  { optLinearFunctionalPatterns :: Bool }
  deriving Show
...
defaultOptimizationOpts = OptimizationOpts
  { optLinearFunctionalPatterns = True }
...
optimizeDescriptions =
  [ ( "optimize-linear-functional-patterns"   ,
      "Use inverse functions to desugar functional patterns"
    , \ opts -> opts { optLinearFunctionalPatterns = True    })
  , ( "no-optimize-linear-functional-patterns",
      "Use lazy unification to desugar functional patterns"
    , \ opts -> opts { optLinearFunctionalPatterns = False   })
  ]
