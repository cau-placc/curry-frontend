...
bindType (IClassDecl _ _ cls _ _ _ ms hs) =
...
checkIDecl (HidingClassDecl p cx qcls k clsvars fdps) = do
  checkTypeVars "hiding class declaration" clsvars
  cx' <- checkClosedContext clsvars cx
  checkSimpleContext cx'
  return $ HidingClassDecl p cx' qcls k clsvars fdps
checkIDecl (IClassDecl p cx qcls k clsvars fdps ms hs) = do
  ...
  return $ IClassDecl p cx' qcls k clsvars fdps ms' hs
...
checkIMethodDecl tvs (IMethodDecl p f a qty) = do
  ...
  checkForAmbigiousType f tvs qty' 
  let QualTypeExpr _ cx _ = qty'
  checkForConstrainedClassVariable f tvs cx
  return $ IMethodDecl p f a qty'

checkForAmbigiousType :: Ident -> [Ident] -> QualTypeExpr -> ISC ()
checkForAmbigiousType _ []       _    = return ()
checkForAmbigiousType f (tv:tvs) qty' =
  if tv `elem` fv qty'
  then checkForAmbigiousType f tvs qty' 
  else report $ errAmbiguousType f tv

checkForConstrainedClassVariable :: Ident -> [Ident] -> Context -> ISC ()
checkForConstrainedClassVariable _ []       _  = return ()
checkForConstrainedClassVariable f (tv:tvs) cx =
  if tv `elem` fv cx
  then report $ errConstrainedClassVariable f tv
  else checkForConstrainedClassVariable f tvs cx
...
checkInstanceType inst = do
  ...
  unless (all isSimpleType inst &&
    ...
    null (filter isAnonId $ concatMap typeVars inst) &&
...
checkClosedContext tvs cx = do
  cx' <- checkContext cx
  mapM_ (\(Constraint _ _ tys) -> mapM (checkClosed tvs) tys) cx'
  return cx'
...
errIllegalSimpleConstraint c@(Constraint _ qcls _) = spanInfoMessage qcls $ vcat
  ...
  , text "the form C u_1 ... u_n, where C is a type class and"
  , text "u_1,...,u_n are type variables."
  ]

-- TODO : Display Span INfo
errIllegalInstanceType :: InstanceType -> Message
errIllegalInstanceType inst = message $ vcat
  [ text "Illegal instance type" <+> hsep (map pPrint inst)
