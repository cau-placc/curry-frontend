...
desugarModuleDecls :: [Decl Type] -> DsM [Decl Type]
...
dsRecordDecl :: Decl Type -> DsM [Decl Type]
...
genSelFun :: SpanInfo -> [QualIdent] -> Ident -> DsM (Decl Type)
...
genSelEqn :: SpanInfo -> Ident -> QualIdent -> DsM [Equation Type]
genSelEqn p l qc = do
  vEnv <- getValueEnv
...
dsDeclRhs :: Decl Type -> DsM (Decl Type)
...
dsEquation :: Equation Type -> DsM (Equation Type)
...
constrain :: [Expression Type] -> Expression Type -> Expression Type
...
dsRhs :: (Expression Type -> Expression Type)
      -> Rhs Type -> DsM (Rhs Type)
...
dsNonLinearity :: [Pattern Type]
               -> DsM ([Expression Type], [Pattern Type])
dsNonLinearity ts = do
  ((_, cs), ts') <- mapAccumM dsNonLinear (Set.empty, []) ts
  return (reverse cs, ts')

type NonLinearEnv = (Set.Set Ident, [Expression Type])

dsNonLinear :: NonLinearEnv -> Pattern Type
            -> DsM (NonLinearEnv, Pattern Type)
...
dsNonLinearFuncPat :: NonLinearEnv -> Pattern Type
                   -> DsM (NonLinearEnv, Pattern Type)
...
mkStrictEquality :: Ident -> (Type, Ident) -> Expression Type
...
dsFunctionalPatterns
  :: SpanInfo -> [Pattern Type]
  -> DsM ([Decl Type], [Expression Type], [Pattern Type])
...
type LazyBinding = (Pattern Type, (Type, Ident))

elimFP :: [LazyBinding] -> Pattern Type
       -> DsM ([LazyBinding], Pattern Type)
...
genFPExpr :: SpanInfo -> [(Ident, Int, Type)] -> [LazyBinding]
...
dsLiteralPat :: Type -> Literal
             -> Either (Pattern Type) (Pattern Type)
...
dsAs :: SpanInfo -> Ident -> ([Decl Type], Pattern Type)
     -> ([Decl Type], Pattern Type)
...
-}
dsExpr :: SpanInfo -> Expression Type -> DsM (Expression Type)
...
dsCase :: SpanInfo -> CaseType -> Expression Type -> [Alt Type]
       -> DsM (Expression Type)
dsCase p ct e alts
...
dsAltRhs :: Alt Type -> DsM (Alt Type)
dsAltRhs (Alt p t rhs) = Alt p t <$> dsRhs id rhs

expandAlt :: (Type, Ident) -> CaseType -> [Alt Type]
          -> DsM (Alt Type)
...
dsDo :: [Statement Type] -> Expression Type -> DsM (Expression Type)
dsDo sts e = foldrM dsStmt e sts

dsStmt :: Statement Type -> Expression Type -> DsM (Expression Type)
...
conType :: QualIdent -> ValueEnv -> ([Ident], Type)
...
varType :: QualIdent -> ValueEnv -> Type
